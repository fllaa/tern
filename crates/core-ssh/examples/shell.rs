...
// SIGWINCH-based resize makes this example unix-only; the Windows resize path
// (console events) lands with the real UI in Phase 1.
...
fn main() {
    eprintln!(
        "the shell example is unix-only for now (SIGWINCH resize); Windows support lands in Phase 1"
    );
}
